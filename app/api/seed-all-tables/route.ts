...
export async function POST() {
  try {
    ...
    const payrollDetailData = {
      ...testData.payrollDetail,
      payroll_id: payroll.id,
      created_at: testData.payrollDetail.createdAt,
      updated_at: testData.payrollDetail.updatedAt
    };
    const { error: payrollDetailError } = await supabase
      .from('payroll_details')
      .insert(payrollDetailData);
    ...
    const { error: deliveryStatsError } = await supabase
      .from('delivery_stats')
      .insert(deliveryStatsData);
